...
class InvertedResidual(nn.Module):  #MV2
    """
    ...
    def __init__(
        ...
        out_channels: int,
        stride: int,
        expand_ratio: Union[int, float],  #将特征图通道数翻expand_ratio倍
        skip_connection: Optional[bool] = True,  #是否使用捷径分支
    ) -> None:
        assert stride in [1, 2]
        hidden_dim = make_divisible(int(round(in_channels * expand_ratio)), 8)  #通过1x1卷积层之后将通道数翻到多少

        super().__init__()

        block = nn.Sequential()
        if expand_ratio != 1: #特征图通道数翻倍
            block.add_module(
                name="exp_1x1",
                module=ConvLayer(
                    in_channels=in_channels,
...
class MobileViTBlock(nn.Module):
    ...
    def __init__(
        self,
        in_channels: int,
        transformer_dim: int,  #输入到transformer_encoder中每个token对应的序列长度
        ffn_dim: int, #MLP中第一个全连接层结点个数
        n_transformer_blocks: int = 2,#transformer_encoder的个数
        head_dim: int = 32,
        attn_dropout: float = 0.0,
        dropout: float = 0.0,
        ffn_dropout: float = 0.0,
        patch_h: int = 8,  #划分后的patch的高
        patch_w: int = 8,  #划分后的patch的宽
        conv_ksize: Optional[int] = 3, #局部表征卷积核的大小
        *args,
        **kwargs
    ) -> None:
        super().__init__()
        ...
        ] #构建n_transformer_blocks个transformer_encoder
    ...
    def unfolding(self, x: Tensor) -> Tuple[Tensor, Dict]:
        ...
        new_h = int(math.ceil(orig_h / self.patch_h) * self.patch_h) #math.ceil：向上取整
        new_w = int(math.ceil(orig_w / self.patch_w) * self.patch_w)

        interpolate = False
        if new_w != orig_w or new_h != orig_h: #为了保证特征图能完整得被分patches
            # Note: Padding can be done, but then it needs to be handled in attention function.
            x = F.interpolate(x, size=(new_h, new_w), mode="bilinear", align_corners=False) #给特征图插值到能够被完整得分patches
            interpolate = True
